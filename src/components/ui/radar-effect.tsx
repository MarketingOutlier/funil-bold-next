      ...
      {circles.map((_, idx) => (
        <Circle
          style={{
            height: `${(idx + 1) * 5}rem`,
            width: `${(idx + 1) * 5}rem`,
            border: `1px solid rgba(201, 168, 76, ${1 - (idx + 1) * 0.1})`,
          }}
          key={`circle-${idx}`}
          idx={idx}
        />
...
export const IconContainer = ({
  ...
  return (
    ...
    >
      <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-white/10 bg-[#16161F] shadow-[0_0_15px_rgba(201,168,76,0.1)] group-hover:border-[#C9A84C]/50 transition-colors duration-300">
        {icon || (
          <svg className="h-8 w-8 text-[#C9A84C]" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clipRule="evenodd" />
          </svg>
        )}
      </div>
      <div className="hidden rounded-md px-3 py-1.5 md:block bg-black/60 backdrop-blur-md border border-white/5 shadow-xl">
